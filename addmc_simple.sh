#!/bin/bash
# addmc_simple.sh - Creditcoin 모니터링 클라이언트 추가 스크립트 (단순화 버전)
...
# docker-compose.yml 파일을 안전하게 업데이트 (간단한 방식)
update_docker_compose_simple() {
  ...
  if grep -q "  mclient:" docker-compose.yml; then
    if [ "$FORCE" = "true" ]; then
      echo -e "${YELLOW}mclient 서비스가 이미 존재합니다. 파일을 다시 구성합니다...${NC}" >&2
      
      # 기존 mclient 제거하기 위한 임시 파일 생성
      cat docker-compose.yml | grep -v "mclient:" > docker-compose-no-mclient.yml
      
      # mclient 서비스 관련 줄 제거
      awk '{
        if ($0 ~ /mclient:/) {
          skip = 1;
        } else if (skip == 1 && ($0 ~ /^  [^ ]/ || $0 ~ /^[^ ]/ || $0 ~ /^$/)) {
          if ($0 ~ /^  [a-zA-Z0-9_-]+:/ || $0 ~ /^[a-zA-Z0-9_-]+:/ || $0 ~ /^$/) {
            skip = 0;
            print;
          }
        } else {
  ...
  # WebSocket 모드에 따른 추가 설정
  if [ "$WS_MODE" = "custom" ] && [ ! -z "$WS_SERVER_URL" ]; then
    mclient_service="${mclient_service}
  ...
  # networks 섹션 위치 찾기
  local networks_line=$(grep -n "^networks:" docker-compose.yml | cut -d: -f1)
  
  if [ -n "$networks_line" ]; then
    # networks 섹션 바로 앞에 mclient 서비스 삽입
    echo -e "${GREEN}networks 섹션 앞에 mclient 서비스 삽입 중...${NC}" >&2
    
    # 빈 줄 확인 및 추가
    local pre_networks_line=$((networks_line-1))
    local pre_networks_content=$(sed -n "${pre_networks_line}p" docker-compose.yml)
    
    (
      # networks 바로 앞까지 파일 복사
      head -n $((networks_line-1)) docker-compose.yml
      
      # 빈 줄이 없으면 추가
      if [ ! -z "$pre_networks_content" ]; then
        echo ""
      fi
      
      # mclient 서비스 추가
      echo "$mclient_service"
      echo ""
      
      # networks 섹션부터 끝까지 복사
    ...
    echo -e "${YELLOW}networks 섹션이 없습니다. 파일 끝에 mclient 서비스와 networks 섹션 추가 중...${NC}" >&2
    
    # 파일 끝에 빈 줄 확인/추가
    local last_line=$(tail -n 1 docker-compose.yml)
    if [ ! -z "$last_line" ]; then
      echo "" >> docker-compose.yml  # 마지막에 빈 줄이 없으면 추가
    fi
    
    # mclient 서비스 추가
    echo "$mclient_service" >> docker-compose.yml
    
    # creditnet이라는 키워드가 파일에 없을 때만 networks 섹션 추가
    if ! grep -q "creditnet:" docker-compose.yml; then
      echo -e "${YELLOW}networks 섹션이 없어 새로 추가합니다.${NC}" >&2
...
EOF
  ...
  echo -e "${BLUE}Docker Compose 파일 검증 중...${NC}" >&2
  if docker compose config > /dev/null 2>&1; then
    echo -e "${GREEN}Docker Compose 파일이 유효합니다.${NC}" >&2
  else
    echo -e "${RED}경고: Docker Compose 파일에 문제가 있을 수 있습니다.${NC}" >&2
    echo -e "${YELLOW}문제를 해결한 후 다시 시도하세요.${NC}" >&2
  fi
}
...
main() {
  ...
  # docker-compose.yml 파일 업데이트 (단순화된 방식)
  update_docker_compose_simple
