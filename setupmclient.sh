...
# config.py 파일 생성
create_config_py() {
  show_step "config.py 파일 생성"
  
  cat > ./mclient/config.py << 'EOF'
import os
...
class Settings(BaseSettings):
    # 기본 설정
    SERVER_ID: str = Field(default="server1", env="SERVER_ID")
    NODE_NAMES: str = Field(default="node,3node", env="NODE_NAMES")
    MONITOR_INTERVAL: int = Field(default=5, env="MONITOR_INTERVAL")
    
    # WebSocket 설정
    WS_MODE: str = Field(default="auto", env="WS_MODE")  # auto, ws, wss, wss_internal, custom
    WS_SERVER_URL: Optional[str] = Field(default=None, env="WS_SERVER_URL")
    
    # Docker 설정
    CREDITCOIN_DIR: str = Field(default=os.path.expanduser("~/creditcoin-mac"), env="CREDITCOIN_DIR")
...
EOF
  ...
  show_success "config.py 파일이 생성되었습니다."
}

# docker_stats_client.py 파일 생성
create_docker_stats_client_py() {
  show_step "docker_stats_client.py 파일 생성"
  
  cat > ./mclient/docker_stats_client.py << 'EOF'
...
EOF
  ...
  show_success "docker_stats_client.py 파일이 생성되었습니다."
}

# websocket_client.py 파일 생성
create_websocket_client_py() {
  show_step "websocket_client.py 파일 생성"
  
  cat > ./mclient/websocket_client.py << 'EOF'
# websocket_client.py
import asyncio
import json
import logging
...
EOF
  ...
  show_success "websocket_client.py 파일이 생성되었습니다."
}

# main.py 파일 생성
create_main_py() {
  show_step "main.py 파일 생성"
  
  cat > ./mclient/main.py << 'EOF'
# main.py
import asyncio
import logging
import argparse
...
EOF
  ...
  show_success "main.py 파일이 생성되었습니다."
}
